let baza = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRO1hLn8zS-X6-yvwgC9cXDDZR4yyT5bDMfGMV0Ge9Qoq8lpctr4fC4fMdLn0aFLYi94G7JWbrWm7qR/pub?gid=725425028&single=true&output=csv";
...
$(document).ready(function () {
    ...
    newQuiz = function () {
        ...
        questionsPage.hide();
        resultsPage.hide();
        if (set_pitanja==2){cat="dva"}
        else if (set_pitanja==3){cat="tri"}
        else if (set_pitanja==4){cat="cet+pet"}
        let output =
        podatci.filter(employee => employee.razina == cat);
        podatci=output
        shuffle(podatci)
    };
    ...
    generateQuestionAndAnswers = function () {
        ...
        $(".definicija").html("<img src='slike/" +podatci[questionCounter].razina+"/"+ podatci[questionCounter].slika + "'class='slikica2'/>")
        //spajanje krivih i točnih odgovora te generiranje liste pitanja
        let sviNazivi = podatci.map(item => item.naziv).filter(naziv => naziv !== podatci[questionCounter].naziv);
        let kriviOdgovori = [];
    while (kriviOdgovori.length < 3) {
        let randomNaziv = sviNazivi[Math.floor(Math.random() * sviNazivi.length)];
        if (!kriviOdgovori.includes(randomNaziv)) {
            kriviOdgovori.push(randomNaziv);
        }
    }

    // Spajanje točnog i netočnih odgovora te miješanje
    let lista = [...kriviOdgovori, podatci[questionCounter].naziv];
    shuffle(lista);
        ...
        $(".vrijeme").html('<progress style="accent-color: #bb422a;"value="60" max="60" id="pageBeginCountdown"><span id="pageBeginCountdownText">60</p>')
    ...
    getCorrectAnswer = function () {
        correctAnswer = podatci[questionCounter].naziv.replace("<br>", "");
    };
...
function odgovor() {
    ...
    if (document.getElementById("pageBeginCountdown").value == "0") {
        ...
        Swal.fire({
            ...
            html: "<p style='text-align:center; font-size: 1.5em;'><strong>Točan je odgovor: <span style='color:#bb422a; ' >" + podatci[questionCounter].naziv + "</span></strong></p><br><figure><img src='slike/" +podatci[questionCounter].razina+"/"+ podatci[questionCounter].slika + " 'class='slikica2'/> </figure>",
        ...
        if (userSelectedAnswer === correctAnswer) {
            ...
            Swal.fire({
                ...
                html: "<span style='font-size:1.5em'>+" + broj + "</span><br><br><figure><img src='slike/" +podatci[questionCounter].razina+"/"+ podatci[questionCounter].slika + "'class='slikica2'/> </figure></p>",
                showCloseButton: true,
                confirmButtonText: ' dalje',
                backdrop: false,
                allowOutsideClick: false, allowEscapeKey: false
